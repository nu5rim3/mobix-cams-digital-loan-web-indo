...
export default function FinancialApproval(props: IFinancialApprovalProps) {
  ...
  useEffect(() => {
    if (financialDetails.data) {
      ...
      setNewRate(financialDetails.data?.pTrhdTr)
    }
  }, [financialDetails.data])
        ...
        <div>
          ...
          <div className={
              ...
              <Space.Compact style={{ width: '100%' }}>
                {
                  financialDetails?.productDetails?.individualProdYn == "N" ?
                    <InputNumber
                      addonBefore={<p style={{ color: '#102C57', fontWeight: 600 }}>Term (Tenor)</p>}
                      value={newTearm}
                      readOnly={!editTerm}
                      onChange={(e: any) => {
                        setNewTearm(e)
            ...
            <div className='pr-6'>
              <Space.Compact style={{ width: '100%' }}>
                <InputNumber
                  addonBefore={<p style={{ color: '#102C57', fontWeight: 600 }}>Loan Rate</p>}
                  value={newRate}
                  readOnly={!editRate}
                  onChange={(e: any) => {
                    setNewRate(e)
                ...
                <Button type="primary"
                  onClick={() => {
                    ...
                    if (editRate) {
                      if (financialDetails?.productDetails?.minTr >= newRate) {

                        actions.financialDSavePendingUpdate(false)
                        return notification.error({
                          message: `Loan Rate Should be Above Your Approval Loan Rate ${getCurrency(financialDetails?.productDetails?.minTr)}`
                        })
                      } else {
