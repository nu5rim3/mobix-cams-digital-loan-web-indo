...
export const getAllApplications = createAsyncThunk(
  ...
  async (
    data: Parameters<typeof API.appraisalsServices.getAllAppraisals>[0],
    thunkAPI
  ) => {
    try {
...
export const getSecondMeetingAppraisals = createAsyncThunk(
  ...
  async (
    data: Parameters<
      typeof API.appraisalsServices.getSecondMeetingAppraisals
    >[0],
    thunkAPI
  ) => {
    try {
      const response = await API.appraisalsServices.getSecondMeetingAppraisals(
        data
      );
      return response?.data;
    } catch (error) {
...
export const getCustomerData = createAsyncThunk(
  ...
  async (
    arg: Parameters<typeof API.personServices.getPersonByIdAppraisalId>[0],
    thunkAPI
  ) => {
    try {
      ...
      let maritalData: any;
      if (maritalStatus) {
        maritalData = await API.productServices.getMaritalStatusByCode(
          maritalStatus
        );
      }
      return {
...
export const getCustomerContactData = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.stakeholderContact.getPersonContactByIdAppraisalId
    >[0],
    thunkAPI
  ) => {
    try {
      const response =
        await API.stakeholderContact.getPersonContactByIdAppraisalId(arg);
      return response.data;
    } catch (error) {
      const er = error as AxiosError;
      return er?.response
...
export const getCustomerAddressData = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.stakeholderAddress.getPersonAddressByIdAppraisalId
    >[0],
    thunkAPI
  ) => {
    try {
      const response =
        await API.stakeholderAddress.getPersonAddressByIdAppraisalId(arg);
      const areasResponse = await API.productServices.getAllAreas();
      const areas = areasResponse.data;
      return response.data?.map((address: any) => {
        const setArea = areas?.find(
          (area: any) => address?.postalCode == area?.code
        )?.description;
        return {
          ...address,
          area: setArea,
...
export const getBusinessData = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.stakeholderBussiness.getPersonBusinessByIdAppraisalId
    >[0],
    thunkAPI
  ) => {
    try {
      const response =
        await API.stakeholderBussiness.getPersonBusinessByIdAppraisalId(arg);
...
export const getSpouseData = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.stakeholderSpouse.getPersonSpouseByIdAppraisalId
    >[0],
    thunkAPI
  ) => {
    try {
      const response =
        await API.stakeholderSpouse.getPersonSpouseByIdAppraisalId(arg);
      ...
      if (relation) {
        relationshipData = await API.productServices.getRelationByCode(
          relation
        );
      }
      // if(subSector){
...
export const getGuarantorDetails = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.stakeholderClientEles.getPersonclientelesByAppraisalId
    >[0],
    thunkAPI
  ) => {
    try {
      ...
      const response =
        await API.stakeholderClientEles.getPersonclientelesByAppraisalId(arg);

      const customerCenterCode = response.data?.find(
        (type: any) => type.cltType === "C"
      ).fusionCenterCode;

      if (customerCenterCode) {
        centerData = await API.stakeholderClientEles.getCentersByCode(
          customerCenterCode
        );
      }
...
export const getCollateralDetails = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.collateralServices.getAllCollateralsByAppraisals
    >[0],
    thunkAPI
  ) => {
    try {
      const response =
        await API.collateralServices.getAllCollateralsByAppraisals(arg);
      if (response.data.landAndBuildingDtoList?.length) {
        ...
        for (let row of landAndBuilding) {
          ...
          if (row.securityCategory) {
            securityCategory = await API.commnServices.getSecurityCat(
              row.securityCategory
            );
          }

          if (row.legalBindingType) {
            legalBindingType = await API.commnServices.getSecurityOwnType(
              row.legalBindingType
            );
          }
          ...
          if (row.certificateType) {
            certificateType = await API.commnServices.getCertificateType(
              row.certificateType
            );
          }
          if (row.morgType) {
          ...
          row.titleInsurance =
            row.titleInsurance == "Y"
              ? "Yes"
              : row.titleInsurance == "N"
              ? "No"
              : "-";
          row.insuranceOfBuilding =
            row.insuranceOfBuilding == "1"
              ? "Yes"
              : row.insuranceOfBuilding == "0"
              ? "No"
              : "-";
          row.powerOfAttorney =
            row.powerOfAttorney == "1"
              ? "Yes"
              : row.powerOfAttorney == "0"
              ? "No"
              : "-";
          row.certificateType = certificateType?.data.description ?? "-";
        }
      }
      if (response.data.condominiumDtoList?.length) {
        const landAndBuilding = response.data.condominiumDtoList;
      ...
      if (response.data.vehicleDtoList?.length) {
        ...
        for (let row of vehice) {
          ...
          if (row.vehicleType) {
            vehicleType = await API.commnServices.getVehicleType(
              row.vehicleType
            );
          }

          if (row.vehicleModel) {
            vehicleModel = await API.commnServices.getVehicleModel(
              row.vehicleModel
            );
          }
...
export const getCashFlowDetails = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.incomeExpencesServices.getAllCollateralsByAppraisals
    >[0],
    thunkAPI
  ) => {
    try {
      const response =
        await API.incomeExpencesServices.getAllCollateralsByAppraisals(arg);
      return response.data;
    } catch (error) {
      const er = error as AxiosError;
      return er?.response
...
export const getApprovalStepsDetails = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.approvalServices.getAllCombinedStepsByAppraisalId
    >[0],
    thunkAPI
  ) => {
    try {
      const response =
        await API.approvalServices.getAllCombinedStepsByAppraisalId(arg);
      return response.data;
    } catch (error) {
      const er = error as AxiosError;
      return er?.response
...
export const getImageDetails = createAsyncThunk(
  ...
  async (
    arg: Parameters<
      typeof API.documentServices.getAllDocumentsByAppraisalId
    >[0],
    thunkAPI
  ) => {
    try {
      const response = await API.documentServices.getAllDocumentsByAppraisalId(
        arg
      );
      return response.data;
    } catch (error) {
    ...
    builder.addCase(getFinanceDetails.pending, (state, action) => {
      ...
      builder.addCase(getFinanceDetails.fulfilled, (state, action) => {
        ...
        (state.financialDetails.termRates = action.payload.termRates),
          (state.financialDetails.productDetails =
